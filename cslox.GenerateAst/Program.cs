...
class Program
{
    ...
    static void DefineBase(string outputDir, string baseName)
    {
    ...
    static void DefineVisitor(string outputDir, string baseName, List<string> types)
    {
        var writer = new StreamWriter(Path.Combine(outputDir, $"IVisitor.cs").ToString(), false, Encoding.UTF8);
        writer.WriteLine("using cslox.Models;");
        writer.WriteLine();
        writer.WriteLine($"namespace cslox.Models;");
        writer.WriteLine();
